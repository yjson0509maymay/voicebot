...
    # :white_check_mark: 추가: 중복 녹음 처리 방지용 hash
...
            st.session_state["chat"] = []
            st.session_state["message"] = [{"role": "system", "content": "You are a thoughtful assistant. Respond to all input in 25 words and answer in Korean"}]
            # :white_check_mark: 초기화 시 hash도 리셋 → 다음 녹음 새것으로 인식
            st.session_state["last_audio_hash"] = None
            st.session_state["check_reset"] = False
            st.rerun()
...
    with col1:
        st.subheader("질문하기")
        audio = audiorecorder(":studio_microphone: 클릭하여 녹음 시작", ":black_square_for_stop: 클릭하여 녹음 중지", pause_prompt="")
        # :white_check_mark: 오리지널 구조 유지: rerun 없이 col1→col2 순서로 흘러내려감
        # → rerun을 쓰면 col2 렌더링 전에 재시작되어 TTS 타이밍이 꼬임
        if (len(audio) > 0) and (not st.session_state["check_reset"]):
            audio_bytes = audio.export().read()
            st.audio(audio_bytes)
            current_hash = hashlib.md5(audio_bytes).hexdigest()
            # :white_check_mark: 새 녹음일 때만 STT/GPT/TTS 처리
            # → 오리지널은 hash 없어서 같은 녹음을 계속 재처리함
            if current_hash != st.session_state["last_audio_hash"]:
                st.session_state["last_audio_hash"] = current_hash
                if not st.session_state["OPENAI_API"]:
                    st.warning(":warning: 사이드바에 API 키를 먼저 입력해주세요.")
                else:
                    with st.spinner("음성 인식 중..."):
                        question = STT(audio, st.session_state["OPENAI_API"])
                    st.success(f":memo: 인식된 텍스트: {question}")
                    now = datetime.now().strftime("%H:%M")
                    st.session_state["chat"] += [("user", now, question)]
                    st.session_state["message"] += [{"role": "user", "content": question}]
    with col2:
        st.subheader("질문/답변")
        # :white_check_mark: 오리지널과 동일한 구조: col2에서 audio 재참조
...
                with st.spinner(":robot_face: GPT 답변 생성 중..."):
                    response = ask_gpt(
...
                            f'<div style="font-size:0.8rem;color:gray;">{t}</div></div>',
                            unsafe_allow_html=True)
                # :white_check_mark: TTS: rerun 없이 이 시점에 바로 재생 → 오리지널과 동일한 타이밍
                TTS(response)
        else:
